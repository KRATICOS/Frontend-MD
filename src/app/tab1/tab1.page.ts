...
import { IonSearchbar } from '@ionic/angular/standalone'; // Añadir este import
...
export class Tab1Page implements OnInit {
  ...
  @ViewChild('searchbar', { static: false }) searchbar!: IonSearchbar; 
    ...
    mostrarBusqueda = false; // Controlar visibilidad del campo de búsqueda
  searchTerm = ''; 
...
buscarPorNumero() {
  ...
  if (esObjectId) {
    ...
    // Si no es ObjectId, buscar por número de serie
...
this.inventarioServices.buscarPorNumeroSerie(serie).subscribe({
  next: (equipo) => {
    if (equipo && (equipo as any)._id) {
      this.router.navigate(['/reserva'], { queryParams: { id: (equipo as any)._id } });
    } else {
      this.mostrarError('Equipo no encontrado');
    }
  },
  ...
  // MÉTODO NUEVO: Alternar visibilidad del campo de búsqueda
  toggleBusqueda() {
